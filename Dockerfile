...
RUN mkdir -p /etc/UpBackend
ADD src/main/webapp/WEB-INF/classes/main.properties /etc/UpBackend
ADD src/main/resources/hibernate.cfg.xml /etc/UpBackend
ADD src/main/resources/error_http_status.properties /etc/UpBackend
ADD target/UpBackend.war /usr/local/tomcat/webapps/
ADD server.xml /usr/local/tomcat/conf/


RUN apt-get update -y && apt-get install -y procps
RUN mkdir /opt/machine-agent
COPY machineagent-bundle-64bit-linux-4.4.3.1214.zip /opt/machine-agent
WORKDIR /opt/machine-agent
RUN unzip -q machineagent-bundle-64bit-linux-4.4.3.1214.zip

RUN mkdir /opt/appdynamics
COPY AppServerAgent-4.4.3.23079.zip /opt/appdynamics
WORKDIR /opt/appdynamics
RUN unzip -q AppServerAgent-4.4.3.23079.zip
